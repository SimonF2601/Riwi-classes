import {callToApi, printMovies,  printSeeMore,cleanHTML} from "./functions.js"
...
containerCards.addEventListener("click", (event)=>{
    ...
    if(event.target.classList.contains("bx-arrow-back")){
        ...
        mode = "search"
        //callToApi(title, mode)
        callToApi(inputTitle.value, mode)
    }
});


inputTitle.addEventListener("input", (event)=>{
    //Event es igual al evento que ocurrio
    //Tarjet es la etiqueta donde ocurrio el evento
    //Value es el valor del input 
    //console.log(event.target.value);
    
    //Se realizo un BackDrop de forma manual con JAVASCRIPT
    clearTimeout(timer);
    timer = setTimeout(()=>{
        //Lamado a la API
        mode = "search";
        callToApi(event.target.value,mode);
    }, 500);
});
